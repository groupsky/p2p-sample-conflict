...
// process revs sent by send
        recieve: function (revs, branch, allowMerge) {
            ...
            if (!this.branches[branch]) {
                this.branches[branch] = revs[0].parent;
            }
...
// add commits to branch
        ...
        merge: function (branches, meta) { //branches...
            ...
            branches = branches.map(function(branch){
                var id = self.getId(branch);
                if (!id) {
                    console.error('missing branch ', branch);
            ...
            if(!concestor) {
                console.error('don\'t have concestor for :', branches);
        ...
        createStream: function (opts, targetBranchPrefix) {
            return createStream(this, opts, targetBranchPrefix)
        }
    })
